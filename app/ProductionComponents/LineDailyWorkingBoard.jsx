          ...
          <div className="flex flex-wrap items-end gap-4">
            ...
            <div className="space-y-1 relative group/line">
              ...
              <div className="absolute left-0 top-full z-50 mt-0.5 w-36 rounded-lg border border-amber-400 bg-white shadow-lg opacity-0 pointer-events-none group-hover/line:opacity-100 group-hover/line:pointer-events-auto transition-opacity duration-150 max-h-56 overflow-y-auto">
                <div
                  className="px-3 py-1.5 text-[11px] text-slate-400 cursor-pointer hover:bg-slate-50"
                  onMouseDown={() => setSelectedLine("")}
                >
                  — clear —
                </div>
                {lineOptions.map((line) => (
                  <div
                    key={line}
                    onMouseDown={() => setSelectedLine(line)}
                    className={`px-3 py-1.5 text-[12px] font-medium cursor-pointer hover:bg-amber-50 hover:text-amber-900 ${
                      selectedLine === line
...
function HourlyHeaderCard({ header, auth }) {
  ...
  const totalAchievedAll        = hasRecords
    ? recordsDecorated.reduce((sum, rec) => sum + (rec._achievedRounded ?? 0), 0)
    : 0;
  ...
  const previousRecord = previousDecorated.length > 0
  ...
  const handleSave = async () => {
    try {
      ...
      const params   = new URLSearchParams({ headerId: header._id, productionUserId });
  ...
  const handleDayInputSave = async () => {
    try {
      ...
      if (!res.ok || !json.success)
        throw new Error(
          json?.errors?.join(", ") || json?.message || "Failed to update total input."
        );

      setTotalInput(
        json.data?.capacity != null ? String(json.data.capacity) : String(newTotal)
      );
      setDayInput("");
      await refreshWip();
      setMessage(
        ...
        <div className="mt-2 rounded-lg border border-slate-200 bg-slate-50 px-3 py-2 text-[11px] space-y-2">
          <div className="flex flex-wrap items-center gap-3">
            ...
            <input
              ...
              min="0"
              step="1"
              className="input input-xxs input-bordered w-20 h-8 text-[12px] font-bold"
              value={totalInput}
          ...
          <div className="flex flex-wrap items-center gap-3">
            ...
            <input
              ...
              min="1"
              step="1"
              className="input input-xxs input-bordered w-20 h-8 text-[12px] font-bold border-green-500"
              value={dayInput}
